...
namespace EchoesOfTheRuins
{
    ...
    public sealed class CanvasHud : MonoBehaviour
    {
        ...
        private void OnTutorialPresentation(TutorialStage stage, float safeEntryRemaining) { tutorialText.text = safeEntryRemaining > 0f ? "SAFE ENTRY  " + Mathf.CeilToInt(safeEntryRemaining) + "s  |  " + TutorialCopy.Get(stage) : TutorialCopy.Get(stage); tutorialPanel.SetActive(stage != TutorialStage.Complete && (safeEntryRemaining > 0f || stage != TutorialStage.Objective)); }
